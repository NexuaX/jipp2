...
double DoubleMatrix::getElement(size_t n, size_t m) const {
...
DoubleMatrix DoubleMatrix::operator+(const DoubleMatrix & dm2) {
	return this->addMatrices(const_cast<DoubleMatrix &>(dm2));
}

DoubleMatrix DoubleMatrix::operator-(const DoubleMatrix & dm2) {
	return this->subtractMatrices(const_cast<DoubleMatrix &>(dm2));
}

DoubleMatrix DoubleMatrix::operator*(const DoubleMatrix & dm2) {
	return this->multiplyMatrices(const_cast<DoubleMatrix &>(dm2));
}

ostream & operator<<(ostream & output, const DoubleMatrix & dm) {
	output << setprecision(6) << fixed;
	output << dm.rows() << " " << dm.cols() << endl;
	for (size_t i = 0; i < dm.rows(); i++) {
		for (size_t j = 0; j < dm.cols(); j++) {
			output << dm.getElement(i, j) << " ";
		}
		output << endl;
	}
	return output;
